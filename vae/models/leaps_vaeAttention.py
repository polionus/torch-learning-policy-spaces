...
from ..utils import init_gru, TransformerEncoderLayer
from .base_vae import BaseVAE, ModelOutput
from config import Config


class LeapsVAEAttention(BaseVAE):
    ...
    def __init__(self, dsl: DSL, device: torch.device, hidden_size: Union[int, None] = None):
        ...
        self.encoder_gru = nn.GRU(self.num_program_tokens - 1, self.hidden_size)
        init_gru(self.encoder_gru)

        # Override the token encoder to make it divisble by the number of heads
        self.token_encoder = nn.Embedding(self.num_program_tokens, self.num_program_tokens - 1)
        self.encoder_transformer_layer = TransformerEncoderLayer(input_dim=self.num_program_tokens - 1, num_heads=Config.model_num_attention_heads, max_seq_len=self.max_program_length)
    ...
    def encode(self, progs: torch.Tensor, progs_mask: torch.Tensor):
        ...
        enc_progs = self.encoder_transformer_layer(enc_progs)
        enc_progs = enc_progs.transpose(0,1)
