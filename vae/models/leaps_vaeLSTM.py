...
from ..utils import init_gru, init_lstm
from .base_vae import BaseVAE, ModelOutput


class LeapsVAELSTM(BaseVAE):
    ...
    def __init__(self, dsl: DSL, device: torch.device, hidden_size: Union[int, None] = None):
        ...
        self.policy_lstm = nn.LSTM(2 * self.hidden_size + self.num_agent_actions, self.hidden_size)
    ...
    def policy_executor(self, z: torch.Tensor, s_h: torch.Tensor, a_h: torch.Tensor,
        ...
        lstm_hidden = z_repeated.unsqueeze(0)
        ...
        for i in range(1, self.max_demo_length):
            ...
            # gru_out, gru_hidden = self.policy_lstm(gru_inputs, gru_hidden)
            lstm_out, (lstm_hidden, lstm_hidden) = self.policy_lstm(gru_inputs, (lstm_hidden, lstm_cell))

            lstm_out = lstm_out.squeeze(0)
            
            pred_action_logits = self.policy_mlp(lstm_out)
